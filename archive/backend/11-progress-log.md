...
| 18 | Analytics & Marketplace Metrics | [Stage 7](09-stage-growth-and-scale.md) | ✅ Done | 2026-09-04 (featured-listing performance descoped, see note below) |
| 19 | Security Hardening | [Stage 7](09-stage-growth-and-scale.md) | 🟡 In Progress | started 2026-09-04 |
...
**Arch Phase 18 started 2026-09-04 — Stage A only (event taxonomy + full-funnel instrumentation), following the same multi-slice-in-progress convention as Arch Phase 26 (Stage 8) above rather than a full phase entry, since funnel-rate computation, subscription/revenue tracking, and vendor/admin analytics dashboards (the phase's other 3 checklist bullets) are separate, not-yet-started later stages.** All 18 event types in product.md §46 are now captured somewhere: 7 already existed pre-Stage-A (`lead_status_changed`, `shortlist_item_added`/`_removed`, `lead_created`, `review_created`, `vendor_comparison_viewed`, `vendor_profile_viewed`, plus `SearchLog`'s dedicated table for Search); this slice added 6 new server-side `logAnalyticsEvent()` hooks (`search_performed` alongside `SearchLog` in `search.service.ts`, `portfolio_viewed` in `album.controller.ts`, `enquiry_completed` in `enquiry.service.ts`, `checkout_started`/`subscription_cancelled` in `subscription.service.ts`, `payment_completed` in `webhook.service.ts`) and a new public, best-effort `POST /analytics/events` endpoint (`wedhub-backend/src/modules/analytics/`, rate-limited at 120/min/IP) for 5 browser-only events with no server-side hook (Page view, Filter, Vendor impression, Vendor click, Enquiry started, Subscription view — Filter and Vendor impression/click reuse the same endpoint with a distinguishing `eventType`). No schema change — `AnalyticsEvent.eventType`/`vendorId`/`metadata` were already free-text/nullable/`Json?`, so no migration. Frontend: `lib/analytics/track.ts` (sendBeacon-first, fetch-keepalive fallback, posted through the existing same-origin authenticated proxy so a logged-in visitor's events attribute to their real `userId` without the tracker ever touching the access token) wired into a new root-layout-mounted `PageViewTracker.tsx`, `VendorCard.tsx` (now a Client Component — impression on mount, click before navigation), `SortSelect.tsx` (the search page's one genuinely client-side filter control — every other filter is a full server-rendered navigation that already re-triggers `search_performed`), `EnquiryModal.tsx` (form-opened), and `SubscriptionBoard.tsx` (page-viewed on mount). Verified live end-to-end against the real dev DB: `POST /analytics/events` → `202` → row in `analytics_events`; a real single-vendor enquiry submission produced both `lead_created` and exactly one `enquiry_completed`; a real (non-trial) checkout produced `checkout_started` with `isUpgrade: false`; a real immediate cancellation produced `subscription_cancelled`; `search`/vendor-profile/albums `GET`s produced `search_performed`/`vendor_profile_viewed`/`portfolio_viewed`. `payment_completed` (webhook-only, needs a real signed Razorpay payload) was verified by code trace/type-check rather than a live webhook call. See `09-stage-growth-and-scale.md`'s Arch Phase 18 checklist for the full 18-event classification table.

**Arch Phase 18 Stage B done 2026-09-04 — vendor analytics**, closing the "Vendor analytics" half of this phase's third checklist bullet (admin analytics is Stage C, still open). Two real backend sources already existed but had never been unified or fully surfaced: `vendor-analytics.service.ts`'s `getVendorAnalytics()` (profile views/leads/reviews, tier-gated 30/90-day window, backing `GET /vendors/me/analytics`) and `lead.repository.ts`'s `getVendorLeadAnalytics()` (response rate/time, conversion, qualified/won/lost, backing the separate `GET /leads/analytics`, already consumed by the standalone `/vendor/analytics` dashboard page's `AnalyticsBoard.tsx` — confirmed a real, live caller, not dead code, before deciding not to touch its contract). This stage closed the two genuine gaps and merged both sources into one response: (1) **Impressions** — new `countImpressions(vendorId, since)` in `vendor-analytics.repository.ts`, mirroring `countProfileViews`'s exact `AnalyticsEvent.count()` shape over the `vendor_impression` events Stage A started emitting; (2) **Enquiries** — new `countDistinctEnquiries(vendorId, since)`, counting distinct `Lead.enquiryId` values for the vendor rather than the global `enquiry_completed` event (which has no `vendorId` and can't be attributed to one vendor, since one enquiry can fan out to several vendors' leads under `MULTI_VENDOR` routing); (3) **Unification** — `getVendorAnalytics()` now also calls `leadRepository.getVendorLeadAnalytics(vendorId, since)` and merges its fields in. `getVendorLeadAnalytics` gained a new optional `since` parameter (default: all-time) purely so this call site could pass its tier-based window; `GET /leads/analytics`'s own caller still passes no argument, so that endpoint's existing all-time contract, response shape, and its one confirmed real caller are all completely unchanged. `VendorAnalytics` (`wedhub-frontend-app/lib/api/vendor-self.types.ts`) and `/vendor/dashboard/page.tsx`'s stat-card grid (3 → 8 cards, matching the file's existing Tailwind classes exactly) were updated to surface all 7 product.md §46 vendor metrics plus reviews; `averageResponseTimeMs: null` renders "No data yet" rather than a fabricated number. No schema change — purely new queries over the existing `AnalyticsEvent`/`Lead` tables, confirmed via `git status` on `prisma/`. Verified live against the real dev DB with a fresh disposable test vendor (registered, vendor created, seeded with known quantities via Prisma, deleted afterward along with its user/leads/enquiries/events/subscription): `GET /vendors/me/analytics` returned `impressions: 7` (7 seeded), `profileViews: 4` (4 seeded), `enquiries: 4` (4 distinct enquiries, correctly not conflated with `leads: 4` even though they matched 1:1 here), `responseRate: 0.5` (2 contacted / 4 received, hand-verified), `conversionRate: 0.25` (1 won / 4 received, hand-verified), `averageResponseTimeMs: 10799830` (≈3-hour average of two seeded 2h/4h response times, hand-verified), `qualifiedLeads`/`wonLeads`/`lostLeads: 1/1/1` — all exactly matching manual calculation. Also verified: `GET /leads/analytics` independently returns the identical numbers for this vendor (confirming the optional-`since` backward-compat), FREE tier correctly omits `profileViewsByDay`, and granting the vendor an `ACTIVE` PRO subscription correctly flipped the same endpoint to `level: "advanced"`/`windowDays: 90` with `profileViewsByDay` present alongside all the same unified fields. `npx tsc --noEmit` passes cleanly in both `wedhub-backend/` and `wedhub-frontend-app/`; `next build` also succeeds with the new fields wired end-to-end.

**Arch Phase 18 Stage C done 2026-09-04 — admin/platform analytics, closing this phase.** Investigated first rather than assumed: the admin dashboard (`GET /admin/dashboard`, `admin-dashboard.service.ts`'s `getDashboardMetrics()`) already existed and already correctly computed 6 of product.md §46's 9 "Platform analytics" metrics (User acquisition, Lead volume, Revenue, MRR, Conversion, plus Total/Active/Paid vendor counts) — unlike Stage B's vendor dashboard, it was not under-built, so this stage closed only the 4 genuine remaining gaps, all computed from existing tables with **zero schema change** (`git status` on `prisma/` showed no diff): (1) **Vendor acquisition** — new `newVendors: { count, windowDays }`, mirroring `newRegistrations`'s exact shape but counting `Vendor.createdAt` instead of `User.createdAt`; (2) **ARR** — `arr: mrr * 12`, the standard SaaS convention, not separately tracked; (3) **Search demand** — new `searchDemand: { count, windowDays, topKeywords }` aggregating the already-populated `SearchLog` table (30-day volume count + top-10 keyword `groupBy`, keyword-null rows excluded); (4) **Churn** — new `churnRate = cancelledInWindow / activeAtWindowStart` (zero-guarded like `conversionRate`). The churn formula needed real care: `Subscription.cancelledAt` is stamped in *two* different scenarios by `subscription.service.ts`'s `cancelSubscription` — immediately on a hard-cancel, but also the moment a vendor merely *requests* cancel-at-period-end (while `status` stays `ACTIVE` and the vendor keeps the product until the period actually ends). `countCancelledInWindow` therefore requires `status: "CANCELLED"` AND `cancelledAt` in-window, not just a non-null `cancelledAt`, to avoid overstating churn with still-paying vendors who merely clicked cancel. The denominator, `countActiveAtWindowStart`, approximates "still a going concern as of the window's start" as `createdAt < since AND (cancelledAt IS NULL OR cancelledAt >= since)` — a documented proxy, since there's no subscription-status-history table to replay the exact status at a past instant. All 4 fields wired into the same single `getDashboardMetrics()` response (no second admin analytics endpoint — kept `GET /admin/dashboard` the one source of truth, per Stage B's unify-into-one-response precedent). Frontend: `AdminDashboardMetrics` (`wedhub-frontend-app/lib/api/admin.types.ts`) extended; `/admin/dashboard/page.tsx` gained a 4th 4-column stat-card row (ARR, Churn rate, New vendors, Search demand, exact existing card classes) plus a "Top searches" ranked-list panel matching the existing "Recent activity"/"Pending approvals" section's weight — no chart library introduced. **Featured-listing performance** (the third named item in this phase's "Subscription conversion, revenue tracking, featured-listing performance" checklist bullet) was investigated and confirmed genuinely unbuilt: the `featured-listings` module (Arch Phase 13) is pure placement/inventory CRUD with no impression/click/CTR tracking, and Stage A's `vendor_impression`/`vendor_click` events aren't placement-scoped (no `featuredListingId`), so there's no existing data to aggregate this from without new event-pipeline instrumentation — out of this stage's "compute from existing data, zero schema changes" scope. Explicitly descoped rather than silently marked done — see `09-stage-growth-and-scale.md`'s Arch Phase 18 note for the full reasoning, following the exact precedent Arch Phase 17 set for descoping static pages/FAQs while still closing that phase. Verified live against the real dev DB: a disposable admin account (created via `prisma/create-admin.ts`, deleted after) called the real endpoint; a disposable vendor+subscription pair (created 40 days ago, cancelled 5 days ago — i.e. 1 active-at-window-start, 1 cancelled-in-window) produced `churnRate: 1` from the live API, exactly matching the constructed scenario's math; `arr` confirmed exactly `mrr * 12` (both 0 — no ACTIVE subscriptions in current dev data); `searchDemand.count: 143` and its top-keyword ordering (`photography: 39, photo: 14, zzzznonexistentvendorzzz: 9, ...`) cross-checked byte-for-byte against manual `prisma.searchLog.count()`/`groupBy` queries run directly against the DB. All disposable accounts/vendor/subscription deleted afterward; `npx tsc --noEmit` clean in both `wedhub-backend/` and `wedhub-frontend-app/`; `next build` succeeds with `/admin/dashboard` building cleanly.

**This completes Arch Phase 18 (Analytics & Marketplace Metrics) across all three stages — event taxonomy (A), vendor analytics (B), and admin/platform analytics (C) — with featured-listing performance the one explicitly descoped, carried-forward gap, following the same descope-and-close pattern Arch Phase 17 established for static pages/FAQs.**

**Arch Phase 19 started 2026-09-04 — Stage A only (dependency audit, security headers, CORS restrictions), following the same multi-slice-in-progress convention as Arch Phases 18/26 above rather than a full phase entry, since input/authorization/file-upload/webhook-security audits, secret rotation, admin MFA, and abuse/spam/SQL-review (the phase's other 3 checklist bullets) are separate, not-yet-started later stages.** This slice covers the entirety of the first checklist bullet:
- **Dependency audit** — `npm audit` run in both `wedhub-backend/` (11 findings: 1 critical, 4 high, 6 moderate) and `wedhub-frontend-app/` (0 findings). `npm audit fix` (no `--force`) applied to the backend; it resolved 0 of the 11 — investigated rather than assumed clean: every finding traces to `qs` (pulled in transitively via `body-parser`←`express`, and via `deepmerge-ts`←`@prisma/config`←`prisma`), and the latest published `qs@6.16.0` that fixes it is still out of range of the latest `body-parser@1.20.6` (`qs: ~6.15.1`) — the real fix requires `express@5.x` or `body-parser@2.x` (both semver-major) or `prisma`'s dev-only config chain bumping past a pinned `deepmerge-ts` range, none of which a non-force audit fix will touch. This confirms and re-verifies the `deepmerge-ts`/`@prisma/config` finding the Arch Phase 1 notes explicitly flagged as "revisit at Arch Phase 19" (line 224 of this file) — still open, same root cause (major-bump-only). The separate pre-existing `esbuild`/`vite`/`vite-node`/`@vitest/mocker`/`vitest` chain (flagged in the Arch Phase 1 notes as "revisit when Arch Phase 20 is implemented," not 19) is also still open and also major-bump-only (`vitest@5.0.0`) — left as-is here too since fixing it is explicitly Arch Phase 20's concern, not this one's. Neither the `qs` chain nor the `vitest` chain is reachable from the running production server (both are dev/build-time-only dependency paths), so leaving both open does not block this stage's other two deliverables. Frontend has nothing to fix.
- **Security headers** — `helmet()` added to `wedhub-backend/src/app.ts`, applied immediately after `app.disable("x-powered-by")` and before any other middleware, with its library defaults (no hand-rolled CSP — this server is API-only, never serves HTML, so helmet's default CSP is inert defense-in-depth rather than a tuned policy). Verified live: `curl -I` against a running dev server shows `Content-Security-Policy`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Resource-Policy`, `Origin-Agent-Cluster`, `Referrer-Policy`, `Strict-Transport-Security`, `X-Content-Type-Options`, `X-DNS-Prefetch-Control`, `X-Download-Options`, `X-Frame-Options`, `X-Permitted-Cross-Domain-Policies`, `X-XSS-Protection` on every response. On the frontend, `wedhub-frontend-app/next.config.ts` gained a real `headers()` function (X-Content-Type-Options, X-Frame-Options: DENY, Referrer-Policy, Permissions-Policy, and a genuine, non-default Content-Security-Policy) since this is the app that actually serves HTML to browsers — the CSP's external allowances were verified against real usage, not guessed: `https://checkout.razorpay.com` (+ `api.razorpay.com`/`lumberjack.razorpay.com` for its documented iframe/telemetry behavior) for `CheckoutButton.tsx`/`PublishCheckoutButton.tsx`'s `<Script src="https://checkout.razorpay.com/v1/checkout.js">`, and `images.unsplash.com` + the real R2 public bucket host (`pub-7116e74b9a3d44a1ab03594911f56ad8.r2.dev`) for `img-src`, matching `next.config.ts`'s own pre-existing `images.remotePatterns`. No nonce/`strict-dynamic` (would force every page to dynamic rendering per Next.js's own CSP docs); `'unsafe-inline'` on script-src/style-src is the same pragmatic no-nonce tradeoff Next's docs describe, needed for Next/React/Tailwind's inline styles. Verified live: `curl -I` on a running dev server (home page and `/search`) shows all 5 headers with the exact expected CSP string; `next build` succeeds cleanly with the config in place (51/51 routes generated).
- **CORS restrictions** — `wedhub-backend/src/app.ts` now applies the `cors` middleware (installed alongside `@types/cors`) right after `helmet()`, restricted to exactly `[env.FRONTEND_URL, env.ADMIN_URL]` (both real env vars from `config/env.ts`, no wildcard) with `credentials: true` — required because auth is httpOnly cookies sent cross-origin from the frontend's Next.js Route Handlers. Verified live against a running dev server: a request with `Origin: http://localhost:3000` (the real `FRONTEND_URL` default) gets back `Access-Control-Allow-Origin: http://localhost:3000` + `Access-Control-Allow-Credentials: true`; the identical request with `Origin: https://evil.com` gets **no** `Access-Control-Allow-Origin` header at all (on both a simple GET and an OPTIONS preflight), which is what makes a browser block the untrusted origin from reading the response / ever sending the real request.
- `npx tsc --noEmit` passes cleanly in both `wedhub-backend/` and `wedhub-frontend-app/`. Remaining Arch Phase 19 checklist bullets (input validation/authorization/file-upload/webhook-security audits; secret rotation/password policy/session revocation/admin MFA; audit-log verification/abuse detection/spam protection/SQL review/production error redaction) are separate, not-yet-started stages — this slice closes only the "Dependency audit, security headers, CORS restrictions" bullet.
...
**Status:** ✅ Done — Stage A started/done 2026-09-04, Stage B done 2026-09-04, Stage C done 2026-09-04, closing the phase the same day it started. Featured-listing performance explicitly descoped (see Notes below).
**Stage:** [Stage 7 — Growth & Scale](09-stage-growth-and-scale.md)

### What this unlocks

Every event in product.md §46's 18-item full-funnel taxonomy now lands in `AnalyticsEvent` (or `SearchLog` for Search), vendors get a single unified analytics endpoint covering all 7 named vendor metrics, and admins get all 9 named platform metrics on the existing `GET /admin/dashboard` — User acquisition, Vendor acquisition, Search demand, Lead volume, Revenue, MRR, ARR, Churn, Conversion. This closes product.md §46 end to end except for featured-listing performance (no placement-attributed impression/click data exists yet — see Notes) and the finer-grained step-by-step funnel-conversion-rate breakdown (search→profile, profile→enquiry, etc. — not itself a named product.md §46 metric, a superset ambition beyond the phase's actual scope).

### APIs completed

| Method | Path | Purpose | Auth |
|---|---|---|---|
| POST | `/api/v1/analytics/events` | Best-effort browser-event ingestion (page view, filter, vendor impression/click, enquiry started, subscription viewed) — rate-limited 120/min/IP | None (best-effort, attributes to `userId` if authenticated) |
| GET | `/api/v1/vendors/me/analytics` | Unified vendor analytics: impressions, profile views, enquiries, leads, response rate/time, conversion, qualified/won/lost, reviews | VENDOR (self) |
| GET | `/api/v1/leads/analytics` | Pre-existing standalone lead analytics (response rate/time, conversion, qualified/won/lost) — unchanged contract, `since` param now optional-and-reusable internally | VENDOR (self) |
| GET | `/api/v1/admin/dashboard` | Unified platform/admin dashboard — all fields listed below | ADMIN |

`GET /admin/dashboard`'s full response shape after Stage C:

```
totalUsers, newRegistrations: { count, windowDays }
totalVendors, newVendors: { count, windowDays }        ← Stage C
activeVendors, paidVendors
totalLeads, totalEnquiries, conversionRate
revenue: { total, thisMonth }
mrr
arr                                                      ← Stage C, = mrr * 12
searchDemand: { count, windowDays, topKeywords[] }       ← Stage C
churnRate                                                ← Stage C
```

### Tables created

None. This entire phase — all three stages — was built with **zero schema changes**, confirmed via `git status` on `prisma/` after every stage's changes. `AnalyticsEvent` (`eventType`/`vendorId`/`metadata` already free-text/nullable/`Json?`), `SearchLog`, and `Subscription.cancelledAt` all already existed and already carried real data; every metric in this phase is a new query over pre-existing tables, not a new table.

### Flow

```
Stage A — instrumentation:
Every funnel action (search, view, click, enquiry, checkout, cancel, ...)
     │
     ├─ server-side action ──► logAnalyticsEvent() called inline in the
     │                          existing service/controller (best-effort,
     │                          never blocks the real request)
     │
     └─ browser-only action ─► lib/analytics/track.ts (sendBeacon) ──►
                                POST /analytics/events ──► logAnalyticsEvent()
     │
     ▼
AnalyticsEvent rows (+ SearchLog for Search specifically)

Stage B — vendor rollup:
GET /vendors/me/analytics
     │
     ├─ countImpressions/countProfileViews (AnalyticsEvent, by eventType)
     ├─ countDistinctEnquiries (distinct Lead.enquiryId for this vendor)
     └─ leadRepository.getVendorLeadAnalytics(vendorId, since) merged in
     ▼
One unified response, tier-gated window (30d FREE / 90d paid)

Stage C — platform rollup:
GET /admin/dashboard
     │
     ├─ pre-existing: totalUsers, newRegistrations, totalVendors,
     │   activeVendors, paidVendors, totalLeads, totalEnquiries,
     │   conversionRate, revenue, mrr
     │
     ├─ new: newVendors            = Vendor.count(createdAt >= 30d ago)
     ├─ new: arr                   = mrr * 12
     ├─ new: searchDemand          = SearchLog.count + groupBy(keyword) top 10
     └─ new: churnRate             = cancelledInWindow / activeAtWindowStart
     ▼
One unified response — same single endpoint, no new route
```

### Notes

- **Stage A, B, and C shipped the same day (2026-09-04)** — unusually fast for a 3-stage phase, because Stage C's investigation found the admin dashboard was already well-built (unlike Stage B's vendor dashboard, which had real gaps to close from near-scratch): 6 of 9 platform metrics already existed before Stage C touched anything, narrowing its real scope to 4 additions.
- **Churn formula required real judgment, not just a plug-in-the-numbers computation** — see this file's Stage C paragraph above (and `admin-dashboard.repository.ts`'s code comments) for why `cancelledAt IS NOT NULL` alone was rejected as the churn numerator (it also fires on a mere cancel-at-period-end *request*, while the vendor is still `ACTIVE` and paying) in favor of `status: "CANCELLED" AND cancelledAt` in-window, and why the denominator is a documented "active as of window start" proxy rather than a full status-history replay (no such history table exists).
- **Featured-listing performance is the one named checklist item this phase does NOT deliver**, investigated and confirmed genuinely unbuilt rather than assumed: `featured-listings` (Arch Phase 13) is placement/inventory CRUD only, and Stage A's impression/click events carry no placement attribution (`featuredListingId`), so there's no existing data to aggregate from without new event-schema work — out of this phase's "compute from existing data, zero schema change" boundary. Tracked as a standalone backlog item, same treatment as Arch Phase 17's static-pages/FAQs carve-out.
