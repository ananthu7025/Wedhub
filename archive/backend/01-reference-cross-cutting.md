...
- **Partial updates under `exactOptionalPropertyTypes`**: `tsconfig.json` has this flag on, which means a plain object with `field: undefined` is a different (disallowed) type from one where `field` is simply absent — and Prisma's generated `create`/`update` input types don't accept explicit `undefined` for an optional field. When building a partial-update payload from a partially-filled DTO (e.g. a PATCH endpoint), strip `undefined` values before passing to Prisma rather than passing them through. See the `omitUndefined`/`DefinedFields<T>` helper in `src/modules/users/users.repository.ts` (Arch Phase 3) for the pattern — reuse it rather than reinventing per module.
