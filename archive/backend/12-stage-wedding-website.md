...
- [x] `WeddingWebsite`/`WeddingWebsiteEvent`/`WeddingWebsiteRsvp` Prisma models, `Payment.purpose`/`Payment.weddingWebsiteId`, `Media.weddingWebsiteId`/`MediaType.WEDDING_WEBSITE_PHOTO`, migration — ✅ Done 2026-09-03 (migration `20260903133532_add_wedding_website`)
- [x] Shared `generateUniqueSlug` helper extracted to `slug.util.ts`; `vendor.service.ts`/`categories.service.ts` refactored to use it — ✅ Done 2026-09-03. Found and fixed 2 more copies than originally scoped (`locations.service.ts`, `vendor-admin.service.ts` also had their own) — all 4 now share one helper, verified live (category and admin-vendor creation both still generate correct `-2`-suffixed slugs on collision).
- [x] `wedding-website` module: draft CRUD (create/get/update), scoped to `ownerUserId` OR `ownerTelegramUserId` — never both, never neither — ✅ Done 2026-09-03. Ownership generalized into a single `WeddingWebsiteOwnerRef` discriminated union (`{kind:"USER"|"TELEGRAM_USER", id}`) threaded through every mutation function, rather than duplicating each into `ForUser`/`ForTelegramUser` pairs — avoids repeating the copy-paste pattern already fixed once for `generateUniqueSlug` above.
- [x] Template listing endpoint (3 seeded templates: Royal Wedding, Minimal Elegant, Traditional Indian Wedding) — template is data-driven (a `template` enum column selecting a renderer), not 3 separate implementations — ✅ Done 2026-09-03, `GET /wedding-websites/templates`
- [x] `wedding-website-media` module: upload-request/confirm for cover photo, couple photo, gallery — reuses R2 + processing queue unmodified — ✅ Done 2026-09-03, verified live (real R2 signed upload, real worker-generated medium/thumbnail WebP variants)
- [x] Preview generation endpoint: enforces the one-free-preview rule (`previewUsedAt` check), generates `generateOpaqueToken()` + `hashToken()`, sets `previewCreatedAt`/`previewExpiresAt` (`now + WEDDING_WEBSITE_PREVIEW_EXPIRY_MINUTES`), marks `previewUsedAt` — ✅ Done 2026-09-03, `POST /wedding-websites/me/:id/preview`
- [x] Public preview-read endpoint — ✅ Done 2026-09-03, `GET /wedding-websites/preview/:token`. **A real info leak was found and fixed during live verification**: the initial implementation returned the raw Prisma row, which includes every scalar column regardless of the repository's `include` — `ownerUserId`, `ownerTelegramUserId`, and `previewTokenHash` were all present in the public response. Fixed with an explicit `toPublicRenderView()` projection (also applied to the published-website read below), re-verified live that none of those fields appear anymore.
- [x] Razorpay order-creation endpoint for the ₹49 charge (`purpose: WEDDING_WEBSITE`), reusing `razorpay.client.ts`'s `createOrder` unmodified — ✅ Done 2026-09-03, `POST /wedding-websites/me/:id/publish-order`, verified live against Razorpay's real test-mode API (real `order_...` id returned)
- [x] Webhook `payment.captured` branch extended: dispatches to `publishWeddingWebsite(weddingWebsiteId)` when `payment.purpose === "WEDDING_WEBSITE"` — ✅ Done 2026-09-03, verified live end-to-end with a real HMAC-signed webhook payload: draft correctly transitioned to `PUBLISHED`, real slug generated (`priya-weds-rahul`), `previewTokenHash` cleared, `publishedAt` set. Idempotency also verified live: resending the identical signed payload correctly deduped at the `WebhookEvent` unique-constraint level (confirmed only one `webhook_events` row exists, `publishedAt` unchanged after the resend).
- [x] Public published-website read endpoint — ✅ Done 2026-09-03, `GET /wedding-websites/published/:slug`, same leak-fixed projection as the preview endpoint
- [x] RSVP submit endpoint + owner-facing RSVP list endpoint — ✅ Done 2026-09-03, verified live (public submit against a real published site, owner-only read, cross-owner access correctly 404s)
- [x] `WEDDING_WEBSITE_PRICE_INR` (default 49) and `WEDDING_WEBSITE_PREVIEW_EXPIRY_MINUTES` (default 60) added to `env.ts` — ✅ Done 2026-09-03
- [x] Telegram flow: `TelegramConversation.flowType`, all 9 `WW_*` states wired into `advanceConversation` (split into `advanceEnquiryConversation`/`advanceWeddingWebsiteConversation`, dispatched by `flowType`), `START` state's second button (`"💍 Create Your Wedding Website – ₹49"`), progressive photo-upload handling mid-conversation, handoff to the same `wedding-website`/`wedding-website-media` service functions the web app uses (via the `WeddingWebsiteOwnerRef{kind:"TELEGRAM_USER"}` path — not a parallel implementation) — ✅ Done 2026-09-03. Two real design gaps found and resolved during this work, both requiring a schema/API decision beyond what was originally scoped:
  - **Photos**: Telegram has no browser, so the web flow's two-phase signed-URL-PUT-confirm upload doesn't apply. Added `downloadTelegramFile()` (`telegram.client.ts`, resolves a `file_id` via `getFileLink` then fetches the bytes server-side) and `ingestTelegramPhoto()` (`wedding-website-media.service.ts`, single-call: downloads, validates size, uploads directly to R2 via the existing `uploadObject`, creates the `Media` row, enqueues the same processing worker) — reuses R2/the processing queue unmodified, per the "Photo ownership" decision above.
  - **Payment**: Checkout.js needs a browser and can't run inside Telegram. Added Razorpay **Payment Links** (a separate API from Orders+Checkout.js) — `createPaymentLink()` (`razorpay.client.ts`) returns a real hosted `rzp.io` short URL sent as a Telegram inline `url` button (added a `url` variant to `InlineButton`, which previously only supported `callbackData`). A Payment Link's payment fires a *different* webhook event, `payment_link.paid`, correlated back to our `Payment` row via the link's own id (echoed in `notes`) rather than a pre-known `order_id` — required a real schema addition, `Payment.razorpayPaymentLinkId` (nullable, unique; migration `20260903150000_add_payment_link_id`), and a new `webhook.service.ts` case (`handlePaymentLinkPaid`) alongside the existing `payment.captured` handler.
  - **Proactive publish notification**: since `payment_link.paid` arrives as a server-to-server webhook with no open conversation turn, added `notifyWeddingWebsitePublished()` (`telegram.webhook.service.ts`) — pushes a "🎉 your website is live" message with a `url` button directly via `telegramProvider.sendMessage` (bypassing the normal request/response conversation-turn pattern) and advances the conversation to `WW_PUBLISHED`. `publishWeddingWebsite()`'s return type changed from `void` to `{ slug }` to support this.
  - **Live-verified end-to-end** via a temporary script driving `advanceConversation` directly against the real dev DB (deleted after use, not committed): full state walk START → `WW_SELECTING_TEMPLATE` → ... → `WW_AWAITING_PAYMENT` → published, asserting real row state at each step. Confirmed live: `WeddingWebsite`/`WeddingWebsiteEvent`/`Media` rows created correctly with `ownerTelegramUserId` set and `ownerUserId` null; a real R2 photo upload; a real Razorpay **test-mode** Payment Link created (`https://rzp.io/rzp/...`); `publishWeddingWebsite` + `notifyWeddingWebsitePublished` correctly transition the website to `PUBLISHED` and the conversation to `WW_PUBLISHED`. One real bug caught live by this: the initial implementation synthesized a placeholder phone number (`"9999999999"`) for Razorpay's `customer.contact` field (Telegram never exposes a real one without a separate, unbuilt share-contact flow) — Razorpay's test-mode API rejected it outright ("Recurring digits in customer contact are disallowed"). Fixed by making `contactPhone` optional end-to-end and omitting `customer.contact` entirely when absent, rather than sending a fake number that happens to pass validation. The bot's own `sendMessage` push could not be fully verified against a real Telegram chat in this environment (no live chat id available to this script) — the call was made with real credentials and correctly rejected Telegram's "chat not found" for the synthetic id used, confirming the code path executes, but a real end-user chat was not observed receiving the message.
- [x] Admin read-only visibility: count, owner, template, payment status, website status, created/published date — ✅ Done 2026-09-03, `GET /admin/wedding-websites`, verified live (real owner email, real payment status/amount, both a DRAFT and a PUBLISHED row visible)
- [x] Venue location (address text + Google Maps link, both independently optional) and a "With the blessings of" parents section — ✅ Done 2026-09-03. No schema change: `venueAddress`/`googleMapsUrl`/`brideParents`/`groomParents` already existed on `WeddingWebsite` from the original migration but were under-used — the web wizard already had all four fields wired (parents inputs upgraded from single-line `<input>` to multi-line `<textarea>` to fit a "names + house name" block, e.g. `"Mr T.R John & Mrs Santha John\nThayyalakkal House"`), but `WeddingWebsiteRenderer` never actually displayed `brideParents`/`groomParents` — added a new blessings section (rendered only if at least one side is filled). Telegram's `WW_COLLECTING_VENUE` state extended from a single free-text prompt into a 3-part sequence (name → address → Maps link, mirroring the web form) and a new `WW_COLLECTING_BLESSINGS` state added (bride's parents → groom's parents) between venue and events — migration `20260903165100_add_ww_collecting_blessings_state` (new `TelegramConversationState` enum value only, no other schema change). Live-verified end-to-end against the real dev DB: full venue sequence and blessings sequence, confirming `venueName`/`venueAddress`/`googleMapsUrl`/`brideParents`/`groomParents` all persist correctly (including that the parents' embedded newline survives the round trip).
