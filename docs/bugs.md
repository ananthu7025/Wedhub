...
### 2. [ ] RBAC tables exist but are completely unused — `authorize()` only checks the legacy role enum
**Status**: Deferred — user decision 2026-09-03. Documenting only, not fixing in this pass; needs its own scoping discussion before implementation (extend `authorize()` to check the AdminUser → Role → Permission chain vs. leave as legacy-role-only and mark the RBAC admin UI as decorative for now).
**Where**: `wedhub-backend/src/middleware/authorize.middleware.ts` (~line 5-19).
**Bug**: `authorize()` does `allowedRoles.includes(req.user.role)` — a hardcoded check against the `User.role` enum (`END_USER`/`VENDOR`/`ADMIN`). The `Role`, `Permission`, `RolePermission`, and `AdminUser` tables (`schema.prisma:107-155`) are fully modeled and the admin UI (Phase 10) lets admins create custom roles/permissions and assign `AdminUser` rows — but none of it does anything. Every admin route still just checks `role === ADMIN`, so custom roles/permissions created in the admin UI are pure decoration.
**Fix**: This is the biggest item here — needs a scoping decision, not a quick patch. Options: (a) extend `authorize()` to accept a permission key, look up the caller's `AdminUser` → `Role` → `Permission` chain, and fall back to the legacy role check for non-admin routes; (b) explicitly document RBAC UI as "not yet enforced" and descope the enforcement work. Recommend deciding scope before touching code — flagging for a explicit go-ahead rather than starting immediately.
