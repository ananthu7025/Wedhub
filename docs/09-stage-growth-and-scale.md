...
- [ ] **Curated "Real Wedding Stories" / case-study content model** — needed to back `REAL_WEDDING_STORIES` in the same file (also a hardcoded placeholder). Fields needed: couple names or a display title, location, a tag/category line (e.g. "South Indian Traditional · 120 Photos"), cover image, snippet/summary, and a way to link to a fuller photo album or story page — likely relates to real `VendorAlbum`/`Media` records (Frontend/Backend Arch Phase 5/6) for an actual wedding, rather than being purely editorial.
- [ ] **Curated "Popular Searches" / trending-search content model** — needed to back `POPULAR_SEARCH_CARDS` in the same file (hardcoded placeholder: title, location blurb, indicative price, image, link). Could be editorial (admin-curated) or analytics-driven (derived from real search volume once Arch Phase 18 exists) — decide which when this is prioritized, don't assume.
- [ ] **Inspiration gallery content model** — needed to back `GalleryInspiration.tsx`'s `GALLERY_ITEMS` (hardcoded placeholder: category, title, image, tag). Consider whether this should be purely editorial CMS content or should surface real vendor portfolio `Media`/`Album` records (Frontend/Backend Arch Phase 5/6) tagged by category — a real-media-sourced version would need no separate CMS model at all, just a query/filter over existing data. Decide at implementation time.
- [ ] These four items were found by cross-referencing the public homepage's actual rendered content against this checklist during a 2026-09-03 UI redesign pass — not previously itemized at this level of detail. They are additions to this existing phase's scope, not a new phase (per this file's own Post-MVP section note: don't invent new Arch Phases speculatively).
