...
### Arch Phase 18 — Analytics & Marketplace Metrics
- [x] **Event tracking across the full funnel: visitor → search → vendor view → enquiry → lead → contact → qualified → won** — ✅ Stage A done 2026-09-04 (aggregation/funnel-rate computation is Stage B, still open — see the two unchecked bullets below). All 18 event types from product.md §46 land in the existing `AnalyticsEvent` model (`wedhub-backend/prisma/schema.prisma` ~line 899) via the existing best-effort `logAnalyticsEvent()` helper (`wedhub-backend/src/common/utils/analytics.util.ts`) — either from an existing server-side hook, a newly-added one, or a new public `POST /analytics/events` endpoint (`wedhub-backend/src/modules/analytics/`, rate-limited 120/min/IP via `analyticsEventRateLimiter`) that the frontend calls for browser-only events. No schema change — `eventType`/`vendorId`/`metadata` were already free-text/nullable/`Json?`.
