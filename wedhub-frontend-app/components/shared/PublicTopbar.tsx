...
export async function PublicTopbar({ variant = "brand", activeHref }: PublicTopbarProps) {
  ...
  return (
    <div className="sticky top-0 z-50 w-full shadow-md">
      ...
      {/* Main Navigation Bar */}
      <header
        className={`flex h-[66px] items-center justify-between px-3 sm:px-6 lg:px-10 transition-colors ${
          isBrand
      ...
      >
            ...
            {session ? (
              ...
              <>
                <Link
                  href="/vendors"
                  className={`rounded-full px-3.5 py-1.5 text-sm font-semibold transition-all ${
                    activeHref === "/vendors" ? "bg-white/25 text-white" : "text-white/90 hover:bg-white/15 hover:text-white"
                  }`}
                >
                  Vendors
                </Link>
                ...
                <Link
                  href="/real-weddings"
                  className={`rounded-full px-3.5 py-1.5 text-sm font-semibold transition-all ${
                    activeHref === "/real-weddings" ? "bg-white/25 text-white" : "text-white/90 hover:bg-white/15 hover:text-white"
                  }`}
                >
                  Real Weddings
                </Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <Link
            href="/search"
            className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-white/15 text-white transition-colors hover:bg-white/25 sm:hidden shrink-0"
            aria-label="Search"
          >
          ...
          {!session && (
            <Link
              href="/reviews/write"
              className="hidden items-center gap-1.5 rounded-full border border-white/40 bg-white/10 px-3.5 py-1.5 text-xs font-bold text-white transition-all hover:bg-white hover:text-[#e00b41] sm:inline-flex shrink-0"
            >
          ...
          {session ? (
            ...
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              <Link
                href="/login"
                className="rounded-full px-2.5 py-1.5 sm:px-4 sm:py-1.5 text-xs sm:text-sm font-bold text-white transition-colors hover:bg-white/20 whitespace-nowrap shrink-0 leading-none"
              >
                Log in
              </Link>
              <Link
                href="/signup"
                className="inline-flex items-center justify-center rounded-full bg-white px-3 sm:px-4 py-1.5 text-xs sm:text-sm font-bold text-[#e00b41] shadow-xs transition-all hover:bg-white/90 hover:shadow whitespace-nowrap shrink-0 leading-none"
              >
                Sign up
              </Link>
            </div>
      ...
      {/* Mobile Category Quick Scroll — maintains exact same items as the navbar */}
      <div className="flex overflow-x-auto border-t border-white/15 bg-[#c2185b] px-3 py-1.5 text-xs font-medium text-white/90 whitespace-nowrap no-scrollbar lg:hidden">
        {session ? (
          <>
