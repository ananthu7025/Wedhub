...
/**
 * Real file-picker upload for a GalleryCategory's pinned homepage cover
 * photo — same R2 presign -> PUT -> confirm flow as CategoryImagePicker.tsx
 * (Category.imageUrl), through the admin-only, platform-owned media-uploads
 * endpoints, since a gallery category has no vendorId to own the upload.
 * onUploaded is called with the resolved url immediately so the parent can
 * persist it via the normal PATCH /admin/gallery-categories/:id save button,
 * same as the name field next to it.
 */
export function GalleryCategoryImagePicker({
  currentImageUrl,
  onUploaded,
}: {
  ...
  async function handleFileSelect(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const requestResult = await createAdminGalleryCategoryImageUploadRequest({
      filename: file.name,
      mimeType: file.type,
      fileSize: file.size,
    });
    ...
    let confirmed = await confirmAdminGalleryCategoryImageUpload(mediaId);
    for (let attempt = 0; attempt < 10 && confirmed.success && !confirmed.data.url; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      confirmed = await confirmAdminGalleryCategoryImageUpload(mediaId);
    }
  ...
  return (
    <div>
      <div className="flex items-center gap-3">
        ...
        >
          {uploading ? "Uploading…" : currentImageUrl ? "Change cover" : "Upload cover"}
        </button>
      </div>
