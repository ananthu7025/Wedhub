...
/**
 * Real file-picker upload for a BlogPost's cover image — same R2
 * presign -> PUT -> confirm flow as PopularSearchImagePicker.tsx (which
 * this closely mirrors), through the admin-only, platform-owned
 * blog-cover-image-upload-requests endpoints (Arch Phase 17, added
 * 2026-09-04) since a blog post, like a Category/PopularSearchCard, has
 * no vendorId to own the upload. The confirm step already returns a
...
export function BlogCoverImagePicker({
  ...
  async function handleFileSelect(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const requestResult = await createAdminBlogCoverImageUploadRequest({
      filename: file.name,
      mimeType: file.type,
      fileSize: file.size,
    });
    ...
    let confirmed = await confirmAdminBlogCoverImageUpload(mediaId);
    for (let attempt = 0; attempt < 10 && confirmed.success && !confirmed.data.url; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      confirmed = await confirmAdminBlogCoverImageUpload(mediaId);
    }
