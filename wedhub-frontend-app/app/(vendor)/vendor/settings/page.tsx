...
import { getMyEffectivePlan, getMyRuleBook, listCategoriesSelf, listLocationsSelf } from "@/lib/api/vendor-self";
...
export default async function VendorSettingsPage() {
  ...
  const [{ data: preferences }, { data: me }, { data: categories }, { data: cities }, { data: ruleBook }, { data: plan }] =
    await Promise.all([
  ...
  return (
    <VendorShell activeHref="/vendor/settings" vendorName={vendor.businessName}>
      <SettingsBoard
        ...
        hasRuleBookAccess={Boolean(plan?.features.rule_book_access)}
      />
    </VendorShell>
  );
}
