...
1. **Wedding website creation wizard** — multi-step (Template → Details → Events → Photos → Story → Preview → Payment → Published — 8 steps; the feature spec's own summary lists 7, but its detailed walkthrough treats Story as distinct, see Open Questions), progress indicator, mobile-first. ✅ Done 2026-09-03 (`components/wedding-website/WeddingWebsiteWizard.tsx`).
...
5. **Admin read-only visibility** — ✅ Done 2026-09-03. New minimal `(admin)/admin/wedding-websites` list page (count, couple/slug, owner email or Telegram name, template, status badge, most recent payment status badge, created/published dates) — no edit/delete/moderate actions, no pagination UI (a single generous page size for now), per the feature spec's explicit "do not build a large admin system" instruction. New `AdminShell.tsx` nav entry under "Platform", next to SEO/CMS/Settings.
...
- [x] Couple story step (story, bride/groom descriptions; optional how-we-met) — ✅ Done 2026-09-03. Added as its own 8th wizard step (`StoryStep`, between Photos and Preview) rather than folded into Details/Photos — the feature spec's own summary progress list shows 7 steps, but its detailed walkthrough treats "Couple Story" as a distinct section; confirmed with the user to give it its own step and its own progress-indicator dot. Verified live: fields save via `PATCH /me/:id` and render correctly on the real preview page.
...
- **Couple story step: resolved 2026-09-03** — added as its own 8th wizard step, diverging from the feature spec's literal 7-step summary progress indicator (its detailed walkthrough already treated Story as distinct, so this is the more faithful reading, not an invention).
...
- **Admin visibility screen: resolved 2026-09-03** — built as its own minimal list page (`(admin)/admin/wedding-websites`) rather than folded into an existing admin page, since none of the existing admin sections (CMS, SEO, dashboard) are a natural fit for a distinct entity list.
