...
- **Recommendation:** Do not decide now; revisit when Frontend Arch Phase 4 is actually reached. Two realistic options: (a) request a small, in-character backend addition — a couple-scoped `GET /enquiries/mine` (or similar) that joins `Enquiry` + the caller's own `Lead` rows via `userId`, since `Enquiry.userId` is already captured at creation time for authenticated submitters; or (b) ship Phase 4's tracker as an explicit "coming soon" / empty state if adding a backend endpoint is out of scope for this pass, documented honestly rather than faked. Whichever is chosen, note it plainly in that phase's `11-progress-log.md` entry.
- **Status:** Open — surfaced during Frontend Arch Phase 3 research (this phase didn't need the endpoint, Phase 4 will), needs a decision when Stage 2's Phase 4 is reached.
- **Related stage files:** [04-stage-couple-experience.md](04-stage-couple-experience.md)
- **Related stage files:** [04-stage-couple-experience.md](04-stage-couple-experience.md)
