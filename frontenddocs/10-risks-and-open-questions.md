...
## 15. No Razorpay test-mode credentials available for checkout verification

- **Citations:** `wedhub-backend/src/integrations/payment/razorpay.client.ts` (`getClient()` throws `ExternalServiceError` if `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` are unset — no mock/sandbox provider exists in the codebase), `wedhub-backend/.env.example` (empty placeholders, no dummy values)
- **Description:** Razorpay checkout requires a real test-mode key pair (`rzp_test_...`) from an actual Razorpay account — there is no backend-side fake/stub payment provider to develop or verify against offline, and this project has no such account available.
- **Impact:** `POST /subscriptions/me/upgrade` for a non-trial plan can be verified up to real Razorpay order creation (confirmed working via curl during Frontend Arch Phase 7), but the actual Checkout.js payment UI, its success/failure callbacks, and the webhook-driven activation (`POST /webhooks/razorpay`) cannot be exercised end-to-end in this environment.
