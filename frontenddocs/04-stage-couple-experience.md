...
- A logged-in couple can shortlist vendors, compare a selection, submit an enquiry, see it appear in their enquiry tracker, get notified when a vendor responds (per whatever notification channel Arch Phase 14 actually delivers to the frontend — likely in-app/email, confirm), and write a review after a qualifying interaction. **Partially done.** Shortlist, compare, and enquiry submission are ✅ Frontend Arch Phase 3, verified 2026-09-02 against two real APPROVED vendors (built through the actual backend API, including a second vendor created specifically for this phase to test the same-category comparison constraint) — full flow (favorite → view on /shortlist → select 2 → /compare shows real distinct data; enquiry modal prefilled from `GET /users/me` → real `POST /enquiries/single-vendor` → success) verified via headed Playwright, 6/6 passing. Enquiry tracker, response notifications, and reviews remain Frontend Arch Phase 4 scope.
...
- [Open Question 11](10-risks-and-open-questions.md#11-no-list-my-enquiries-endpoint-exists-for-the-couple-side) — no couple-scoped enquiry-list endpoint exists in the backend at all. Blocks Frontend Arch Phase 4's `couple/leads.html` port until resolved (either a new backend endpoint or a different data source).
