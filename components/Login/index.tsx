...
import NxtSvg from "../Svg/NextBtnSvg/indx";
...
const LoginForm = () => {
  ...
  const handleSubmit = async (e: any) => {
    try {
      ...
      if (data) {
        toaster.success(messages[0]);
        const { userId, email, role, isEmailVerified, profileCompleted } = data;

        const route = !isEmailVerified
          ? `/otp?id=${userId}&email=${email}&role=${role}`
          : !profileCompleted
        ...
        setTimeout(() => {
          ...
          return router.push(generatePath(lang, route));
        }, 1000);
      }
    } catch (error) {
      setLoading(false);
    ...
    <div className="col-lg-7">
      <div className="login-sec">
        <div className="logo-head-wrap">
          ...
          <span>
            New to WedHub?
            <Link href={generatePath(lang, "/vendor-signup")}>Signup</Link>{" "}
          </span>
        </div>
        ...
        <button disabled={loading} onClick={handleSubmit} className="theme-btn">
          Login
          {loading ? (
        ...
        <div className="forget-wrap mt-2">
